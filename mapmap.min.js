      ...
      1: [
        function(require, module, exports) {
          ...
          function rowFileHandler(loader) {
            return function(path, map, reduce, options) {
              ...
              return new Promise(function(resolve, reject) {
                loader(path, options.accessor, function(error, data) {
                  if (error) {
                    reject(error)
                    return
                  }
          ...
          var dd = function(spec, map, reduce, options) {
            options = options || {}
            if (spec == null)
              throw new Error('datadata.js: No data specification.')
            if (map && !dd.isFunction(map)) {
              map = dd.map.key(map)
            }
            if (dd.isString(spec)) {
              var handler =
                options.fileHandler || getFileHandler(options.type || spec)
              if (handler) {
                ...
                var obj = values.reduce(function(prev, curr) {
                  ...
                  for (var i = 0; i < keys.length; i++) {
                    ...
                    if (
                      ...
                      if (doAdd) {
                        console.warn(
                          'datadata.emit.sum(): Cannot add keys ' + key + '!'
                        )
                      }
                    }
                  }
      ...
      3: [
        function(require, module, exports) {
          ...
          function assert(test, message) {
            if (test) return
            throw new Error('[mapmap] ' + message)
          }
          assert(window.d3, 'd3.js is required!')
          assert(
            window.Promise,
            'Promises not available in your browser - please add the necessary polyfill, as detailed in https://github.com/floledermann/mapmap.js#using-mapmapjs'
          )
          var default_settings = {
            locale: 'en',
            keepAspectRatio: true,
            placeholderClassName: 'placeholder',
            svgAttributes: { overflow: 'hidden' },
            pathAttributes: {
              fill: 'none',
              stroke: '#000',
              'stroke-width': '0.2',
          ...
          var mapmap = function(element, options) {
            ...
            this._promise = { geometry: null, data: null }
            this.selected = null
          ...
          mapmap.datadata = dd
          mapmap.prototype = { version: version }
          ...
          mapmap.prototype.initEngine = function(element) {
            ...
            this._elements = {
              ...
              backgroundGeometry: mapEl
                .append('g')
                .attr('class', 'background-geometry'),
              background: mapEl
                .append('rect')
                .attr('class', 'background')
                .attr(this.settings.backgroundAttributes),
              ...
              placeholder: mainEl.select(
                '.' + this.settings.placeholderClassName
              ),
            }
            this.width = null
            this.height = null
            ...
            this.supports = {}
            var el = this._elements.main
              .append('path')
              .attr({
                'paint-order': 'stroke',
                'vector-effect': 'non-scaling-stroke',
              })
            var val = getComputedStyle(el.node()).getPropertyValue(
              'paint-order'
            )
            this.supports.paintOrder = val && val.indexOf('stroke') == 0
            val = getComputedStyle(el.node()).getPropertyValue('vector-effect')
            this.supports.nonScalingStroke =
              val && val.indexOf('non-scaling-stroke') == 0
            this._elements.main.classed(
              'supports-non-scaling-stroke',
              this.supports.nonScalingStroke
            )
            el.remove()
            if (
            ...
            mapEl.call(this.zoom.event)
            var map = this
            function constructEvent(event) {
              var pos = [event.clientX, event.clientY],
                location = map._projection.invert
                  ? map._projection.invert(pos)
                  : null
              return {
                position: pos,
          ...
          mapmap.prototype.geometry = function(spec, keyOrOptions) {
            var options = dd.isString(keyOrOptions)
              ? { key: keyOrOptions }
              : keyOrOptions
            options = dd.merge({ key: 'id', setExtent: true }, options)
            var map = this
                    ...
                    if (options.key) {
                      for (var i = 0; i < objs.length; i++) {
                        ...
                        if (obj.properties && obj.properties[options.key]) {
                          objs[i].properties.__key__ =
                            obj.properties[options.key]
                        }
                      }
                    }
                  }
              ...
              for (var k = 0; k < properties.length; k++) {
                ...
                for (var i = 0; i < layers.length(); i++) {
                  ...
                  for (var j = 0; j < geoms.length; j++) {
                    ...
                    if (
                      geom.properties &&
                      geom.properties[property] !== undefined &&
                      geom.properties[property].toString().toLowerCase() == name
                    ) {
                      result.push(geom)
                    }
                  }
            ...
            return function(query, callback) {
              map.promise_data().then(function() {
                ...
                for (var i = 0; i < sel.length; i++) {
                  ...
                  if (
                    d[propName] &&
                    d[propName].toLowerCase().indexOf(query.toLowerCase()) == 0
                  ) {
          ...
          mapmap.prototype.draw = function() {
            var groupSel = this._elements.geometry
              .selectAll('g')
              .data(this.layers.keys(), function(d, i) {
                return d
              })
            var map = this
          ...
          mapmap.prototype.select = function(selection) {
            ...
            function getName(sel) {
              return typeof sel == 'string'
                ? sel
                : sel.selectionName || 'function'
            }
            var oldSel = this.selected
            if (this.selected) {
              this._elements.main.classed(
                'selected-' + getName(this.selected),
                false
              )
            }
          ...
          mapmap.prototype.highlight = function(selection) {
            ...
            if (selection === null) {
              ...
              this.promise_data().then(function(data) {
                ...
                map._elements.shadowCropEl.selectAll('path').remove()
                obj.each(function() {
                  map._elements.shadowEl
                    .append('path')
                    .attr({
                      d: this.attributes.d.value,
                      fill: 'rgba(0,0,0,0.5)',
                    })
                  map._elements.shadowCropEl
                    .append('path')
                    .attr({ d: this.attributes.d.value, fill: '#fff' })
                })
              })
            }
            return this
          ...
          mapmap.prototype.data = function(spec, keyOrOptions) {
            var options = dd.isDictionary(keyOrOptions)
              ? keyOrOptions
              : { map: keyOrOptions }
            options = dd.merge({ geometryKey: '__key__' }, options)
            var map = this
            if (typeof spec == 'function') {
              this.promise_data().then(function(data) {
                map._elements.geometry.selectAll('path').each(function(geom) {
                  if (geom.properties) {
              ...
              this.promise_data(
                dd(spec, options.map, options.reduce, options)
              ).then(function(data) {
                if (data.length() == 0) {
                  console.warn(
                    "Data for key '" + options.map + "' yielded no results!"
                  )
                }
                map._elements.geometry.selectAll('path').each(function(d) {
                  if (d.properties) {
          ...
          var MetaDataSpec = function(key, fields) {
            if (!(this instanceof MetaDataSpec))
              return new MetaDataSpec(key, fields)
          ...
          MetaDataSpec.prototype.match = function(str) {
            ...
            var rex = new RegExp(
              '^' + this.key.replace('*', '.*').replace('?', '.')
            )
            return str.search(rex) == 0
          }
          var MetaData = function(fields, localeProvider) {
            if (!(this instanceof MetaData))
              return new MetaData(fields, localeProvider)
            mapmap.extend(this, fields)
            if (this.undefinedLabel === undefined)
              this.undefinedLabel = localeProvider.locale.undefinedLabel
            ...
            this.getFormatter = function() {
              ...
              if (localeProvider.locale) {
                return localeProvider.locale.numberFormat(
                  this.numberFormat || '.01f'
                )
              }
            ...
            this.getRangeFormatter = function() {
              ...
              return function(lower, upper, excludeLower, excludeUpper) {
                if (localeProvider.locale && localeProvider.locale.rangeLabel) {
                  return localeProvider.locale.rangeLabel(
                    lower,
                    upper,
                    fmt,
                    excludeLower,
          ...
          mapmap.prototype.autoColorScale = function(
            value,
            metadata,
            selection
          ) {
            ...
            if (!metadata.domain) {
              var stats = getStats(
                this.getRepresentations(selection),
                properties_accessor(keyOrCallback(value))
              )
              if (stats.anyNegative && stats.anyPositive) {
                metadata.domain = [
                  Math.min(stats.min, -stats.max),
                  Math.max(stats.max, -stats.min),
                ]
              } else {
            ...
            scale
              ...
              .range(metadata.color || metadata.colors)
          ...
          mapmap.prototype.autoLinearScale = function(valueFunc) {
            var stats = getStats(
              this._elements.geometry.selectAll('path'),
              properties_accessor(valueFunc)
            )
            return d3.scale.linear().domain([0, stats.max])
          }
          mapmap.prototype.autoSqrtScale = function(valueFunc) {
            ...
            return d3.scale.sqrt().domain([0, stats.max])
          }
          ...
          mapmap.prototype.zOrder = function(comparator, options) {
            options = dd.merge({ undefinedValue: Infinity }, options)
            ...
            var map = this
            this.promise_data().then(function(data) {
              map.getRepresentations().sort(comparator)
            })
            return this
          }
          mapmap.prototype.symbolize = function(callback, selection, finalize) {
            ...
            this.promise_data().then(function(data) {
              map.getRepresentations(selection).each(function(geom) {
          ...
          mapmap.prototype.symbolizeAttribute = function(
            property,
            reprAttribute,
            options
          ) {
            ...
            var defaultUndefinedAttributes = { stroke: 'transparent' }
            ...
            this.promise_data().then(function(data) {
              ...
              map.symbolize(function(el, geom, data) {
                el.attr(reprAttribute, function(geom) {
                  ...
                  if (
                    val == null ||
                    (metadata.scale != 'ordinal' && isNaN(val))
                  ) {
                    return (
                      (metadata.undefinedSymbols &&
                        metadata.undefinedSymbols[reprAttribute]) ||
          ...
          mapmap.prototype.choropleth = function(spec, metadata, selection) {
            this.symbolize(
              mapmap.symbolize.choropleth(spec, metadata, selection),
              selection,
              function() {
            ...
            function color(el, geom, data) {
              ...
              el.attr('fill', function(geom) {
                ...
                if (
                  ...
                  return (
                    metadata.undefinedColor || map.settings.pathAttributes.fill
                  )
                }
          ...
          mapmap.symbolize.addLabel = function(spec, textAttributes) {
            ...
            return function(el, geom, data) {
              ...
              if (
                geom.properties &&
                typeof valueFunc(geom.properties) != 'undefined'
              ) {
                ...
                this.getOverlayPane()
                  ...
                  .attr(textAttributes)
                  .attr({ x: centroid[0], y: centroid[1] })
              }
            }
          }
          ...
          function addOptionalElement(elementName) {
            return function(value) {
              ...
              return function(el, d) {
                ...
                el.append(elementName).text(valueFunc(d.properties))
              }
            }
          }
          ...
          mapmap.prototype.getAnchorForRepr = function(event, repr, options) {
            ...
            if (options.clipToViewport) {
              if (pt.x < mapBounds.left + options.clipMargins.left)
                pt.x = mapBounds.left + options.clipMargins.left
              if (pt.x > mapBounds.right - options.clipMargins.right)
                pt.x = mapBounds.right - options.clipMargins.right
              if (pt.y < mapBounds.top + options.clipMargins.top)
                pt.y = mapBounds.top + options.clipMargins.top
              if (pt.y > mapBounds.bottom - options.clipMargins.bottom)
                pt.y = mapBounds.bottom - options.clipMargins.bottom
            }
            pt.x -= mapBounds.left
            pt.y -= mapBounds.top
            return pt
          }
          mapmap.prototype.getAnchorForMousePosition = function(
            event,
            repr,
            options
          ) {
            options = dd.merge({ anchorOffset: [0, -20] }, options)
            ...
            this.promise_data().then(function() {
              ...
              mouseover = function(d) {
                ...
                window.setTimeout(function() {
                  var anchor = options.anchorPosition.call(
                    map,
                    event,
                    el,
                    options
              ...
              if (overCB) {
                obj.on('mouseenter', mouseover).each(function() {
                  var sel = d3.select(this)
            ...
            var func = function(d) {
              ...
              for (var i = 0; i < spec.length; i++) {
                ...
                if (part) {
                  ...
                  var prefix =
                    meta.hoverLabel || meta.valueLabel || meta.label || ''
                  if (prefix) prefix += ': '
          ...
          mapmap.prototype.hoverInfo = function(spec, options) {
            options = dd.merge(
              {
                ...
                hoverEnterStyle: { display: 'block' },
                hoverLeaveStyle: { display: 'none' },
              },
              options
            )
            var hoverEl = this._elements.parent.select(
              '.' + options.hoverClassName
            )
            if (!spec) {
            ...
            if (hoverEl.empty()) {
              hoverEl = this._elements.parent
                .append('div')
                .attr('class', options.hoverClassName)
            }
            ...
            function show(d, point) {
              ...
              if (html) {
                ...
                if (
                  getComputedStyle(offsetEl).getPropertyValue('position') ==
                  'static'
                ) {
                  console.warn(
                    'mapmap.js Warning: hoverInfo parent element needs to be positioned (relative or absolute) for positioning to work properly!'
                  )
                }
                hoverEl
                  .style({
          ...
          mapmap.behavior.zoom = function(options) {
            ...
            var z = function(selection) {
              map = this
              var size = this.size()
              r = (Math.min(size.height, size.width) / 2) * options.ringRadius
              r0 =
                Math.sqrt(size.width * size.width + size.height * size.height) /
                1.5
              if (options.cursor) {
                selection.attr({ cursor: options.cursor })
              }
              if (options.showRing && !ring) {
                ring = map
                  .getFixedPane()
                  .selectAll('g.zoomRing')
                  .data([1])
                var newring = ring
                  .enter()
                  .append('g')
                  ...
                  .attr(
                    'transform',
                    'translate(' +
                      size.width * options.center[0] +
                      ',' +
                      size.height * options.center[1] +
                      ')'
                  )
                newring
                  .append('circle')
            ...
            function animateRing(selection) {
              if (ring) {
                ...
                ring
                  ...
                  .attr(
                    'transform',
                    'translate(' + new_r * 0.707 + ',-' + new_r * 0.707 + ')'
                  )
          ...
          mapmap.prototype.animateView = function(
            translate,
            scale,
            callback,
            duration
            ...
            if (
              translate[0] == this.current_translate[0] &&
              translate[1] == this.current_translate[1] &&
          ...
          mapmap.prototype.zoomToSelection = function(selection, options) {
            ...
            var dx = bounds[1][0] - bounds[0][0],
              ...
              scale = Math.min(
                options.maxZoom,
                options.fitScale / Math.max(dx / size.width, dy / size.height)
              ),
              translate = [
                size.width * options.center[0] - scale * x,
                size.height * options.center[1] - scale * y,
              ]
            this.animateView(
              translate,
              scale,
          ...
          mapmap.prototype.zoomToBounds = function(bounds, callback, duration) {
            var w = bounds[1][0] - bounds[0][0],
              ...
              scale = Math.min(
                2,
                0.9 / Math.max(w / size.width, h / size.height)
              ),
              translate = [
                size.width * 0.5 - scale * cx,
                size.height * 0.5 - scale * cy,
              ]
            return this.animateView(translate, scale, callback, duration)
          }
          mapmap.prototype.zoomToCenter = function(
            center,
            scale,
            callback,
            duration
            ...
            var size = this.size(),
              translate = [
                size.width * 0.5 - scale * center[0],
                size.height * 0.5 - scale * center[1],
              ]
            return this.animateView(translate, scale, callback, duration)
          }
          mapmap.prototype.zoomToViewportPosition = function(
            center,
            scale,
            callback,
            duration
            ...
            point.x = center[0]
            point.y = center[1]
            var ctm = this._elements.geometry
              .node()
              .getScreenCTM()
              .inverse()
          ...
          mapmap.prototype.applyBehaviour = function(spec, selection) {
            console &&
              console.log &&
              console.log(
                'Deprecation warning: applyBehaviour() is deprecated, use applyBehavior() (US spelling) instead!'
              )
          ...
          mapmap.prototype.applyMapBehaviour = function(spec, selection) {
            console &&
              console.log &&
              console.log(
                'Deprecation warning: applyMapBehaviour() is deprecated, use applyMapBehavior() (US spelling) instead!'
              )
          ...
          function defaultRangeLabel(
            ...
            if (isNaN(lower)) {
              if (isNaN(upper)) {
                console.warn(
                  'rangeLabel: neither lower nor upper value specified!'
                )
                return ''
              } else {
          ...
          mapmap.prototype.updateLegend = function(
            attribute,
            reprAttribute,
            metadata,
            scale,
            ...
            if (
              reprAttribute == 'fill' &&
              (metadata.undefinedColor != 'transparent' ||
                (metadata.undefinedSymbols &&
                  metadata.undefinedSymbols.fill != 'transparent'))
            ) {
              undefinedClass = {
                representation:
                  metadata.undefinedColor || metadata.undefinedSymbols.fill,
                class: 'undefined',
                count: counter(null),
          ...
          mapmap.legend.html = function(options) {
            ...
            options.histogramBarWidth = parameterFunction(
              options.histogramBarWidth,
              function(param) {
            ...
            var legend_func = function(
              ...
              var legend = this._elements.parent.select(
                '.' + options.legendClassName
              )
              if (legend.empty()) {
                legend = this._elements.parent
                  .append('div')
              ...
              var title = legend
                .selectAll('h3')
                .data([
                  valueOrCall(metadata.label, attribute) ||
                    (dd.isString(attribute) ? attribute : ''),
                ])
              ...
              if (options.order) {
                assert(
                  dd.isFunction(options.order),
                  'LegendOptions.order must be a comparator function!'
                )
                classes.sort(options.order)
              }
              var cells = legend.selectAll('div.legendCell').data(classes)
              cells.exit().remove()
              var newcells = cells
                .enter()
                .append('div')
                .style(options.cellStyle)
              cells.attr('class', 'legendCell').each(function(d) {
                if (d.class) {
              ...
              function updateRepresentations(newcells, cells, options) {
                ...
                if (reprAttribute == 'fill') {
                  ...
                  var strokeAttrs = {}
                  strokeAttrs[reprAttribute] = function(d) {
                    return d.representation
                  }
                  newcells
                    .append('line')
              ...
              cells.attr('data-count', function(d) {
                return d.count()
              })
              cells.select('.legendLabel').text(function(d) {
                var formatter
                if (d.valueRange) {
              ...
              if (options.histogram) {
                ...
                cells
                  .select('.legendHistogramBar')
                  .transition()
                  .style('width', function(d) {
                    var width = options.histogramBarWidth(d.count())
                    if (
                      width.length &&
                      width.indexOf('px') == width.lenght - 2
                    ) {
                      return width
                    }
            ...
            legend_func.clear = function() {
              this._elements.parent
                .select('.' + options.legendClassName)
                .remove()
            }
            return legend_func
          }
          ...
          mapmap.prototype._extent = function(geom, options) {
            options = dd.merge({ fillFactor: 0.9 }, options)
            if (geom.type && geom.type == 'Topology') {
              var names = Object.keys(geom.objects)
              var all = []
              for (var i = 0; i < names.length; i++) {
                all = all.concat(
                  topojson.feature(geom, geom.objects[names[i]]).features
                )
              }
              geom = all
            }
            if (dd.isArray(geom)) {
              var all = { type: 'FeatureCollection', features: geom }
              geom = all
            }
            ...
            var scale =
              options.fillFactor /
              Math.max(
                bounds.width / size.width / fac_x,
                bounds.height / size.height / fac_y
              )
            this._projection
              .scale(scale)
              .translate([
                (size.width - scale * (bounds[1][0] + bounds[0][0])) / 2,
                (size.height - scale * (bounds[1][1] + bounds[0][1])) / 2,
              ])
            this._elements.map.selectAll('path').attr('d', pathGenerator)
          }
